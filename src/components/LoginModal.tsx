...
const LoginModal: FC = () => {
  ...
  const [fontSize, setFontSize] = useState<number | null>(null); // Добавляем состояние для размера шрифта
  const dispatch = useAppDispatch();
  const open = useAppSelector((state) => state.authReducer.modalOpen);

  useEffect(() => {
    const handleResize = () => {
      // Обновляем размер шрифта в зависимости от ширины окна браузера
  ...
  }, []); // [] указывает на то, что эффект нужно запустить только один раз после монтирования компонента
              ...
              <form onSubmit={submitForm} className="flex flex-col space-y-3">
                <div className="relative">
                  <input
                    ...
                    style={{ fontSize: fontSize }} // Устанавливаем размер шрифта
                  />
